...
if ($day==0 && $year != "")
{
	$subtotal="select sum(total_price) as total from invoice where month(invoice.invoice_date)=$month && year(invoice.invoice_date)=$year && branchID='$branchID'  and void='A' order by inv.invoice_no desc";
   ...
   $day_counter=$row['total'];
	...
	for ($i=0;$i<count($shop_array);$i++){
		$shop_subtotal[$i]="select sum(total_price) as total from invoice where month(invoice.invoice_date)=$month && year(invoice.invoice_date)=$year and branchID='$shop_array[$i]'  and void='A' ";
		$settle_shop_subtotal[$i]="select sum(total_price) as total from invoice where month(invoice.invoice_date)=$month && year(invoice.invoice_date)=$year and branchID='$shop_array[$i]' and settle='A'  and void='A' ";
		$unsettle_shop_subtotal[$i]="select sum(total_price) as total from invoice where month(invoice.invoice_date)=$month && year(invoice.invoice_date)=$year and branchID='$shop_array[$i]' and settle='S'  and void='A' ";
		$unsettle_shop_deposit[$i]="select sum(deposit) as total from invoice where month(invoice.invoice_date)=$month && year(invoice.invoice_date)=$year and branchID='$shop_array[$i]' and settle='S'  and void='A' ";
...
else if ($day!="" && $year != "")
{
	...
	inv.special_man_power_percent   ,	inv.total_price   	,	inv.deposit   	,	inv.settle   	,
	inv.credit_card_rate,mem.member_name
	from member mem RIGHT JOIN invoice inv ON mem.member_id = inv.member_id where  ((month(inv.settledate)=$month && year(inv.settledate)=$year && DAYOFMONTH(inv.settledate)=$day)) && branchID='$branchID'  and inv.void='A' order by inv.invoice_no desc "; 
...
<tr><td colspan="13">
   ...
   <tbody><tr>
     <td>總計 <?=$total_price[$invoice_no_array[$i]]?> 信用卡 <?=$credit_card_rate[$invoice_no_array[$i]]?>% <a href="/?page=invoice&subpage=invoice_edit.php&id=<?php echo $invoice_no_array[$i];?>">&lt;EDIT&gt;</a></td></tr>
