...
<script type="text/javascript">
	$(function() {
		$("#accordion2<? echo $_GET['branchID']; ?>").accordion({ 
...
if ($day==0 && $year != "")
{
	$subtotal="select sum(total_price) as total from invoice_scrap where month(invoice_scrap.invoice_date)=$month && year(invoice_scrap.invoice_date)=$year && branchID='$branchID' order by invoice_no desc";
   ...
   $day_counter=$row['total'];
	...
	for ($i=0;$i<count($shop_array);$i++){
		$shop_subtotal[$i]="select sum(total_price) as total from invoice_scrap where month(invoice_scrap.invoice_date)=$month && year(invoice_scrap.invoice_date)=$year and branchID='$shop_array[$i]'";
		$settle_shop_subtotal[$i]="select sum(total_price) as total from invoice_scrap where month(invoice_scrap.invoice_date)=$month && year(invoice_scrap.invoice_date)=$year and branchID='$shop_array[$i]' ";
...
else if ($day!="" && $year != "")
{
	...
	inv.special_man_power_percent   ,	inv.total_price   	,	inv.deposit   	,	inv.settle   	,
	inv.credit_card_rate,mem.member_name
	from member mem RIGHT JOIN invoice_scrap inv ON mem.member_id = inv.member_id where  ((month(inv.settledate)=$month && year(inv.settledate)=$year && DAYOFMONTH(inv.settledate)=$day)) && branchID='$branchID' order by inv.invoice_no desc "; 
	...
	$sql="select 
	ginv.goods_partno,   ginv.qty ,ginv.unit_price , ginv.subtotal 
	from goods_invoice_scrap  ginv  where  ginv.invoice_no = '$invoice_no_array[$i]'";
...
?>
			<div id="accordion2<? echo $_GET['branchID']; ?>" class="menu noaccordion">
<?            for ($i=0;$i<count($invoice_no_array);$i++){ ?>
...
<tr><td colspan="13">
   ...
   <tbody><tr>
     <td>總計 <?=$total_price[$invoice_no_array[$i]]?>  <a href="../invoice_scrap/invoice_scrap_edit.php?id=<?php echo $invoice_no_array[$i];?>">&lt;EDIT&gt;</a></td></tr>
