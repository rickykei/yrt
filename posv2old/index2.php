	...
	<div  id="page_footage_div">
		...
		$i=0;
		  while ($typerow = $type1Result->fetchRow(DB_FETCHMODE_ASSOC))
		 {
			...
			?>
			<td><a class="ui-button ui-widget ui-corner-all" href="/?pos=<?php echo $pos;?>&page=posv2&subpage=<?php echo $index3_page;?>.php&model1=<?php echo $model1;?>&model2=<?php echo stripslashes($typerow['typeName']);?>&parent_id=<?php echo stripslashes($typerow['id']);?>" rel="external"><?php echo stripslashes($typerow['typeName']);?></a></td>
			 <?php
			  $i++;
			  if ($i==5){
				 echo "</tr>";
