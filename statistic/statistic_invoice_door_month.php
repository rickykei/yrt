...
$subtotal="select sum(total_price) as total from invoice_door where invoice_door.invoice_date>='$date_start' && invoice_door.invoice_date<='$date_end' ";
   ...
   $m_day_counter=$row['total'];
	...
	for ($i=0;$i<count($shop_array);$i++){
		...
		$shop_subtotal[$i]="select sum(total_price) as total from invoice_door where invoice_door.invoice_date>='$date_start' && invoice_door.invoice_date<='$date_end' and branchID='$shop_array[$i]'";
		$settle_shop_subtotal[$i]="select sum(total_price) as total from invoice_door where invoice_door.settledate>='$date_start' && invoice_door.settledate<='$date_end'  and branchID='$shop_array[$i]' ";
...
<table  align="center" >
  ...
  <tr>
   <?
    ...
    <?
  	for ($i=0;$i<count($shop_array);$i++){
		$fixAmt=$m_settle_shop_subtotal_counter[$i];
