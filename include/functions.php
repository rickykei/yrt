	...
	if ($row['mix']=='N'){
		...
		//$masterItemStr=substr($masterItemStr, 1);
		 ...
		 for($i=0;$i<count($masterItem);$i++){
				 ...
				 $invoiceSql="select goods_partno, ifnull(sum(qty),0) as qty from invoice i , goods_invoice gi where deductstock='Y' and  i.invoice_no=gi.invoice_no and goods_partno = '".$masterItem[$i]['partno']."'  and delivery!='W' ";
				  if($isPredict<=0)
					 $invoiceSql=add_del_time_SQL($invoiceSql);
						else
					 $invoiceSql=add_del_time_SQL_by($invoiceSql,$isPredict,$year,$month,$day);
		...
		for ($i=0;$i<count($sameChildItem);$i++)
		{
			...
			$invoiceSql="select ifnull(sum(qty),0) as qty from invoice i , goods_invoice gi where deductstock='Y' and  i.invoice_no=gi.invoice_no and goods_partno = '".$sameChildItem[$i]['partno_src']."'  and delivery!='W' ";
			 if($isPredict<=0)
					 $invoiceSql=add_del_time_SQL($invoiceSql);
						else
					 $invoiceSql=add_del_time_SQL_by($invoiceSql,$isPredict,$year,$month,$day);
		...
		$invoiceSlaveSql="select goods_partno,ifnull(sum(qty),0) as qty from invoice i , goods_invoice gi where deductstock='Y' and  i.invoice_no=gi.invoice_no and goods_partno ='".$slaveItem[$i]['partno']."'  and delivery!='W' ";
		 if($isPredict<=0)
					 $invoiceSlaveSql=add_del_time_SQL($invoiceSlaveSql);
						else
					 $invoiceSlaveSql=add_del_time_SQL_by($invoiceSlaveSql,$isPredict,$year,$month,$day);
		...
		while(  $row2 = $result2->fetchRow(DB_FETCHMODE_ASSOC))
		{
		 ...
		 $scrapSlaveSql= "select goods_partno, ifnull(sum(qty),0) as qty from goods_scrap gs , scrap s where s.invoice_no=gs.invoice_no and goods_partno in ($slaveItemStr) ";
		     if($isPredict<=0)
		 $scrapSlaveSql=add_del_time_SQL($scrapSlaveSql);
			else
		 $scrapSlaveSql=add_del_time_SQL_by($scrapSlaveSql,$isPredict,$year,$month,$day);
