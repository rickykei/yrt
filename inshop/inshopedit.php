...
//get inshop
	...
	$sql="SELECT * FROM inshop where inshop_no=$inshop_no";
	$inshopresult = $connection->query($sql);
	$inshoprow = $inshopresult->fetchRow(DB_FETCHMODE_ASSOC);

//get inshop_goods

	$sql="SELECT * FROM goods_inshop where inshop_no=$inshop_no";
	$inshopresult = $connection->query($sql);
...
<table width="940" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#99CCCC">
  ...
  <tr>
    ...
    <td align="center" valign="top"><table width="101%" height="100%" border="0" cellpadding="2" cellspacing="0">
      <tr>
        <td width="14%" height="21" bgcolor="#006666"><a href="/"><span class="style6">修改入舖單</span></a></td>
      ...
      <tr bgcolor="#FFFFFF">
        <td height="24" colspan="4"><table width="100%" border="0" cellpadding="2" cellspacing="0">
          <tr bgcolor="#006666">
            <td height="21"><label><span class="style6">入舖單編號 :</span> </label></td>
            <td colspan="2"><label><span class="style6"><?php echo $inshoprow['inshop_no']; ?></span><input type="hidden" name="inshop_no" value="<?=$inshoprow['inshop_no'];?>"> </label></td>
            <td width="29%">&nbsp;</td>
          </tr>
          <tr bgcolor="#006666">
            <td width="14%" height="21">
                <span class="style6"> <span class="style5">發票</span>日期 ：</span></td>
            <td width="40%"><input name="inshop_date" type="text" id="inshop_date" value="<? echo $inshoprow['inshop_date']; ?>">
              <input name="cal" id="calendar" value=".." type="button" /></td>
            <td width="17%"><span class="style6">職員 : <?=$inshoprow['staff_name']?>*</span></td>
            <td><select name="staff_name" id="staff_name">
              <?php while ($row = $staffResult->fetchRow(DB_FETCHMODE_ASSOC))
			  {
			  
			  	if ($row['name']==$inshoprow['staff_name'])
          ...
          <tr bgcolor="#006666">
            ...
            <td>
			  ...
			  {
				if ($row['supplier_name']==$inshoprow['supplier_name'])
            ...
            <td><input name="supplier_invoice_no" type="text" id="supplier_invoice_no" value="<? echo $inshoprow['supplier_invoice_no']; ?>" /></td>
          </tr>
        </table></td>
      </tr>
      <tr bgcolor="#FFFFFF">
...
$tab=0;
$bgcolor="#CCCCCC";
while($goods_inshoprow = $inshopresult->fetchRow(DB_FETCHMODE_ASSOC))          
	{
	//20090409
    //add goods_partno searching caiteria
    if ( ( preg_match("/".$goods_detail."/", $goods_inshoprow['goods_detail'], $matches) && $goods_detail!="" ) || $goods_partno==$goods_inshoprow['goods_partno'] || (preg_match("/".$goods_partno2."/", $goods_inshoprow['goods_partno'], $matches)  && $goods_partno2!="") || $market_price == $goods_inshoprow['market_price'])
    {
      $bgcolor="#CCFF##";
    } else {
    	$bgcolor="#CCCCCC";
	...
	?>
          <tr bgcolor="<?=$bgcolor?>">
            ...
            <td><input  name="goods_partno[]" type="text" id="goods_partno<?echo $i;?>" value="<? echo $goods_inshoprow['goods_partno']; ?>" tabindex="<?$tab++;echo $tab?>" size="15" maxlength="30"  />
            ...
            <td><input name="qty[]" type="text" id="qty<?echo $i;?>"  onFocus="javascript:document.getElementById('action<?=$i?>').click();" tabindex="<?$tab++;echo $tab?>"  value="<? echo $goods_inshoprow['qty']; ?>" size="7" maxlength="7"></td>
                       <td><div align="center">
              <input name="goods_detail[]" type="text" id="goods_detail<?echo $i;?>" value="<? echo htmlspecialchars($goods_inshoprow['goods_detail']); ?>" size="35" maxlength="40">
            </div></td>
			 <td><div align="center">
			   <input name="market_price[]" type="text" id="market_price<?echo $i;?>" tabindex="<?$tab++;echo $tab?>" value="<? echo $goods_inshoprow['market_price']; ?>" size="6" maxlength="6">
			 </div></td>
            <td><div align="center"><input name="discount[]" type="text" id="discount<?echo $i;?>" tabindex="<?$tab++;echo $tab?>" value="<? echo $goods_inshoprow['discount']; ?>"  size="8" maxlength="8" /></div></td>
            <td><input name="subtotal[]" type="text" tabindex="<?$tab++;echo $tab?>" id="subtotal<?echo $i;?>"  value="<? echo $goods_inshoprow['subtotal']; ?>"  size="8" maxlength="8" onfocus="javascipt:countSubTotal(<?=$i?>)"/></td>
<td><?php if ($goods_inshoprow['deductstock']=='N'){ ?>
...
for ($y=$i;$y<$inshopRecord;$y++)
{
?>
    <tr bgcolor="#CCCCCC">
            <td><div align="center"><?=$y+1?></div></td>
            <td><input  name="goods_partno[]" type="text" id="goods_partno<?echo $y;?>" value="<? echo $goods_inshoprow['goods_partno']; ?>" tabindex="<?$tab++;echo $tab?>" size="15" maxlength="30"  />
            <input type=button name="search" value=".." onClick="javascript:AddrWindow(<?echo $elements_counter;$elements_counter=$elements_counter+7;?>)" >
        ...
        <td height="" colspan="4">
          <table width="100%" border="0" cellpadding="2" cellspacing="0" bgcolor="#006666">
            <tr>
              ...
              <td><textarea name="remark" cols="65" rows="2" id="remark"><? echo $inshoprow['remark']; ?></textarea></td>
            </tr>
            <tr>
              <td colspan="2">&nbsp;</td>
              <td>&nbsp;</td>
              <td><div align="right" class="style6"><strong>總數 : </strong></div></td>
              <td><input name="count_price" type="text" class="disabled" id="count_price" value="<? echo $inshoprow['count_price']; ?>" size="10" /></td>
              <td></td>
            </tr>
            <tr>
              <td colspan="2">&nbsp;</td>
              ...
              <td><input name="sub_discount" type="text" class="disabled" id="sub_discount" value="<? echo $inshoprow['discount_percent']; ?>" size="10" /></td>
              <td></td>
            </tr>
            <tr>
              <td colspan="2">&nbsp;</td>
              ...
              <td width="10%"><input name="total_price" type="text" class="disabled" id="total_price" value="<? echo $inshoprow['total_price']; ?>" size="10" /></td>
              <td width="1%"></td>
            </tr>
          </table>          </td>
      </tr>
