<?php
   include_once("./include/config3.php");
...
<?php echo "window.opener.document.getElementById('goods_detail".$recid."').value=b;";?>
	...
	//window.opener.findPartNoAjax(<?php echo $recid;?>);
	window.close();
} 
...
<form name="hello" method="post" action="/?page=bonus_by_item&subpage=page_search_partno.php">
<strong>貨品編號</strong>:
...
if ($update==2||$update==3)
{
   ...
   if ($update==2&&$model!="")
   $query1="select * from sumgoods where goods_partno like '%$goods_partno%' and model like '$model%' and status='Y'";
   else if($update==2&&$goods_partno=="" && $model=="" && $price!="")
   $query1="select * from sumgoods where market_price ='$price' order by goods_partno and status='Y'";
   else if ($update==2&&$goods_partno!="" && $model=="" && $price=="")
   $query1="select * from sumgoods where goods_partno like '%$goods_partno%'  and status='Y' order by goods_partno";
   if ($update==3)
   $query1="select * from sumgoods where goods_partno = '$goods_partno' and status='Y'";
