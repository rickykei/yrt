...
<body><?php
  ...
  $sql="SELECT * FROM staff where name !='' ";
   ...
   include('./invoice/Pager_header.php');
...
<form name="search" action="/?page=invoice&subpage=invoicelist_amend.php" method="POST">
...
<TD width="32" bgcolor="#006633">月修改表</td>
   ...
   <td><?=$row['sales_name']?></td>
	...
	?>
   <td><a  href="/?page=invoice&subpage=invoice_edit_amend.php&id=<?=$row['amend_invoice_id']?>">Edit</a></td>
     <td><a  target="_blank" href="/?page=invoice&subpage=invoice_edit_monthly_pdf.php&id=<?=$row['amend_invoice_id']?>&branchID=<?=$row['branchID']?>&date=<?=substr($row['amend_date'],0,10)?>">PDF</a></td>
   </tr>
		 <? }
   ?>
