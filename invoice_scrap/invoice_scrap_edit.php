<?php
  ...
  $ts = new TIMESTAMP;
	...
	//get Staff name	  
	 ...
	 $sql="Select * from invoice_scrap where invoice_no = ".$id;
	...
	$sql="select * from goods_invoice_scrap where invoice_no=".$id." order by id asc";
...
<script type="text/javascript" src="./invoice_scrap/invoice.js?date=20180406"></script>
<link href="./include/invoice.css" rel="stylesheet" type="text/css">
 <form action="/?page=invoice_scrap&subpage=invoice_scrap_edit2.php" method="POST" enctype="application/x-www-form-urlencoded" name="form1">
...
<table width="1000" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#99d6ff">
  <tr>
    ...
    <td  align="center" valign="top"><table width="100%" height="100%" border="0" cellpadding="2" cellspacing="0">
      <tr>
        <td width="13%" height="21" bgcolor="#004d80"><span class="style6"><a href="list.php">方邊門出貨單</a></span></td>
...
for ($i=0;$i<$invoiceRecord;$i++)          
{
	?>
          <tr bgcolor="#CCCCCC">
			...
			<td>
			<input type="text" name="partno[<?echo $i;?>]"  id="partno[<?echo $i;?>]" onKeyPress="next_text_box(event,'width<?=$i;?>')" onChange="findPartNoAjax('<?=$i?>')" value="<?php echo $goods_invoice[$i]['goods_partno'];?>">
			 </td>
			 <td><input name="width[<?echo $i;?>]" type="text" id="width[<?echo $i;?>]" size="5" maxlength="10" value="<?php echo $goods_invoice[$i]['width'];?>" onChange="findPartNoAjax('<?=$i?>')"></td>
            <td><input name="height[<?echo $i;?>]" type="text" id="height[<?echo $i;?>]"  size="5" maxlength="10" value="<?php echo $goods_invoice[$i]['height'];?>" onChange="findPartNoAjax('<?=$i?>')"></td>
		    <td><input name="qty[<?echo $i;?>]" type="text" id="qty[<?echo $i;?>]" size="5" maxlength="5" value="<?php echo $goods_invoice[$i]['qty'];?>" onChange="findPartNoAjax('<?=$i?>')" ></td>
		    <td><div align="center">
...
$</strong></span></td>
              ...
              <td width="5%" bgcolor="#666666">  <input name="submitb" type="submit" id="submitb" value="送出"></td>
            </tr>
          </table>          </td>
      </tr>
    </table></td>
